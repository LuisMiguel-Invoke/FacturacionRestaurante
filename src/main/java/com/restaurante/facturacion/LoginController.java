...
import com.restaurante.facturacion.service.ILoginService;
import com.restaurante.facturacion.service.IResetTokenService;
import com.restaurante.facturacion.util.EmailUtil;
import com.restaurante.facturacion.util.Mail;
	...
	public ResponseEntity<Integer> enviarCorreo(@RequestBody String correo) {
		...
		try {
			...
			if (us != null && us.getIdUsuario() > 0) {
				...
				Mail mail = new Mail();
				mail.setFrom("facturacion.restaurante.11@gmail.com");
				mail.setTo(us.getUsername());
				mail.setSubject("RESTABLECER CONTRASEÑA - FACTURACIÓN RESTAURANTE");
